...
const AGENT_NAMES = [
  "CRAWLER", "SCOUT-01", "SCOUT-02",
  "MINER-01", "MINER-02",
  "SYNTH-01",
]

function generateGraphData(onboarding: OnboardingData): GraphData {
  const nodes: GraphNode[] = []
  const links: GraphLink[] = []

  const hostname = (() => {
    try { return new URL(onboarding.website).hostname.replace("www.", "") }
    catch { return onboarding.website || "your product" }
  })()

  // Core nodes from onboarding data
  nodes.push({
    id: "core-product",
  ...
  nodes.push({
    id: "core-audience",
    name: onboarding.audience.length > 28
      ? onboarding.audience.slice(0, 28) + "…"
      : onboarding.audience || "Target Audience",
    type: "core",
    confidence: 90,
    domain: "audience",
    group: -1,
  ...
  // Trend nodes — content strategy discoveries
  const trendNames = [
    "Hook Patterns", "Sound Trends", "Algorithm Shift",
    "Engagement Spike", "Format Evolution", "Audience Drift",
    "Viral Cascade", "Content Velocity", "Cross-Platform Leak",
    "Creator Economy", "AI Content Wave", "Micro-Community",
    "Long-Form Pivot", "Duet Chains", "Story Arc Format",
    "Educational Shift", "Shoppable Content", "UGC Explosion",
    "Behind-the-Scenes", "Data Storytelling",
  ]
  ...
  // Insight nodes — actionable findings
  const insightNames = [
  ...
  // Core audience → some trends
  for (let i = 0; i < 6; i++) {
    links.push({
      source: "core-audience",
      target: `trend-${Math.floor(Math.random() * trendNames.length)}`,
      strength: 0.5 + Math.random() * 0.3,
    })
  }

  // Core tiktok → some trends
  for (let i = 0; i < 4; i++) {
    links.push({
      source: "core-tiktok",
      target: `trend-${Math.floor(Math.random() * trendNames.length)}`,
...
export function KnowledgeGraph({ visible, onboardingData, onNodeClick }: KnowledgeGraphProps) {
  ...
  const initGraph = useCallback(async () => {
    ...
    const ForceGraph3D = mod.default as unknown as () => (element: HTMLElement) => ForceGraph3DInstance
    const THREE = await import("three")

    const graphData = generateGraphData(onboardingData)
    ...
    graph
      ...
      .nodeThreeObject((node: GraphNode) => {
        ...
        const size = isCore ? 5 : node.type === "agent" ? 4 : node.type === "trend" ? 2.5 : 1.5
        const conf = (node.confidence ?? 50) / 100

        let color: InstanceType<typeof THREE.Color>
        if (isCore) {
          color = new THREE.Color("#d4745f")
        } else {
        ...
        const glowMat = new THREE.MeshBasicMaterial({
          color,
          transparent: true,
          opacity: isCore ? 0.2 : node.type === "agent" ? 0.15 : 0.08,
        })
        const glow = new THREE.Mesh(glowGeo, glowMat)
        group.add(glow)

        // Ring for core & agent nodes
        if (isCore || node.type === "agent") {
          const ringGeo = new THREE.RingGeometry(size * 1.5, size * 1.7, 32)
          const ringMat = new THREE.MeshBasicMaterial({
            color,
            transparent: true,
            opacity: isCore ? 0.4 : 0.3,
        ...
        canvas.width = 256
        canvas.height = 64
        ctx.font = `${isCore || node.type === "agent" ? "bold 14px" : "11px"} 'JetBrains Mono', monospace`
        ...
        const spriteMat = new THREE.SpriteMaterial({
          map: texture,
          transparent: true,
          opacity: isCore ? 1 : node.type === "agent" ? 0.95 : 0.7,
        })
      ...
      .linkColor((link: GraphLink) => {
        ...
        if (src && src.type === "core") return "rgba(212, 116, 95, 0.25)"
        const strength = link.strength ?? 0.3
        const alpha = Math.round(25 + strength * 50)
        return `rgba(160, 160, 160, ${alpha / 255})`
      })
      .linkOpacity(0.3)
      .linkWidth((link: GraphLink) => {
        const src = typeof link.source === "object" ? link.source as GraphNode : null
        if (src && src.type === "core") return 1.5
        return 0.3 + (link.strength ?? 0.3) * 1.2
      })
      .linkDirectionalParticles((link: GraphLink) => {
        const src = typeof link.source === "object" ? link.source as GraphNode : null
        if (src && src.type === "core") return 4
        return src && src.type === "agent" ? 3 : 1
      })
      ...
      .onNodeClick((node: GraphNode) => {
        onNodeClick(node)
        const dist = node.type === "core" ? 140 : node.type === "agent" ? 120 : 80
    ...
    try {
      ...
      if (renderer && scene && camera) {
        const composer = new EffectComposer(renderer as never)
        const renderPass = new RenderPass(scene as never, camera as never)
      ...
      {hoveredNode && (
        ...
        >
          <div className="flex items-center gap-2">
            ...
            />
            <span
              className="uppercase tracking-[0.06em] font-mono"
              style={{ fontSize: "10px", color: "#d0d0d0" }}
            >
              {hoveredNode.name}
            </span>
          </div>
          <p className="font-mono" style={{ fontSize: "9px", color: "#7f8a97", marginTop: 2 }}>
